...
def download_consurf_results(idsList):
	...
	already_downloaded = [(filename[:4],filename[4]) for filename in os.listdir(CONSURF_DB_DIR)]
	
	test_idsList = [(queryId[:4],queryId[4]) for queryId in idsList]
	print("To download:",len(test_idsList))
	downloaded_useful = [name for name in already_downloaded if name in test_idsList]
	print("Already downloaded:",len(downloaded_useful))
	# print(test_idsList[0])
	logfile = os.path.join(OUT_DIR, 'ConSurfDB_download.log')
	failed_downloads=[]
	if os.path.exists(logfile):
		failed_downloads = [(line.split("_")[0][:4],line.split("_")[0][4]) for line in open(logfile, 'r')]

	print("Failed downloads:",len(failed_downloads))
	
	print("Actually trying to download this run:",len([x for x in test_idsList if x not in already_downloaded+failed_downloads]))
	...
	for queryId in tqdm(idsList):
		...
		# # Need to download:
		# # https://consurfdb.tau.ac.il/DB/6TTUT/6TTUT_consurf_summary.txt
		# # queryURL = "https://consurfdb.tau.ac.il/DB/"+queryId+"/consurf_summary.txt"
		# queryURL = "https://consurfdb.tau.ac.il/DB/"+queryId+"/"+queryId+"_consurf_summary.txt"
		# downloaded_file_Name = queryId+"_consurf_summary.txt"
		...
		# 		# If it's a "B" chain, try the "A" one?
		# 		if (chainID == "B"):
		# 			tempqueryId = pdbID+"A"
		# 			tempqueryURL = "https://consurfdb.tau.ac.il/DB/"+tempqueryId+"/"+tempqueryId+"_consurf_summary.txt"
		# 			tempdownloaded_file_Name = queryId+"_consurf_summary.txt"
		# 			tempdownloaded_file_path = os.path.join(CONSURF_DB_DIR, tempdownloaded_file_Name)
		# 			if not os.path.exists(tempdownloaded_file_path):
		# 				os.system('wget -q -O'+tempdownloaded_file_path+' '+tempqueryURL)
		# 				if os.stat(tempdownloaded_file_path).st_size == 0: # If download failed
		# 					log.write(downloaded_file_Name+" -> Unable to find the grades page on ConSurfDB site\n")
		# 					os.remove(tempdownloaded_file_path)
		# 				else:
		# 					hasConsurfResults.append((pdbID,chainID))

		# 		# If it's a "A" chain, try the "B" one?
		# 		elif (chainID == "A"):
		# 			tempqueryId = pdbID+"B"
		# 			tempqueryURL = "https://consurfdb.tau.ac.il/DB/"+tempqueryId+"/"+tempqueryId+"_consurf_summary.txt"
		# 			tempdownloaded_file_Name = queryId+"_consurf_summary.txt"
...
def add_to_protein_models(has_consurf_results,models,paths,names):
	'''
	...
	for i,model,name in zip([0,1,2], models,names):
		...
		if not os.path.exists(logfile):
			...
			for protein in tqdm(modeledProteins):
				ID = protein.pdb[0].upper()+protein.pdb[1] # Format: PDBID_chainID
...
def write_log_file(proteins_with_errors,double_counted,logfile,modeledProteins):
	...
	resNum = {str(protein.orf+", "+protein.pdb[0].upper()+protein.pdb[1]) : len(protein.residues) for protein in modeledProteins}
...
def main():
	...
	idsList,[model_pombe, model_cere] = prep_input([SPOM_PPI_MODELS_PATH,SCER_PPI_MODELS_PATH])
	
	
	hasConsurfResults = download_consurf_results(idsList)
	
	add_to_protein_models(hasConsurfResults,[model_pombe,model_cere],[SPOM_PPI_MODELS_PATH,SCER_PPI_MODELS_PATH],[ "pombe","cere"])

	
	print_summary(model_cere,"PPI models for cerevisiae")
	print_summary(model_pombe,"PPI models for pombe")
