...
class Residue:
	def __init__(self, id, indexInSeq, aaLetter,cdns,num_closely_related,rsaMonomer,rsaComplex,drsa):
		...
		self.consurfDB_score_wrong =  np.nan          	# score using the "wrong" lineage  
		self.consurf_score = np.nan 					 # Manually computed ConSurf score (Unsing rate4site with MSA and tree as input)
		self.consurf_score_wrong = np.nan                  # score using the "wrong" lineage    
...
class PPI:
	...
	def __init__(self,orf1, orf2, reports):
		...
		self.orf2 = orf2
		self.proteins = []
		self.numberOfReports = str(reports)      	# Number of reports for the PPI 
		self.multipleReports = False            	# True of the PPI is reported more than once
		self.ppiType= []
		if(self.numberOfReports) == "Haiyuan":
			self.multipleReports = True  
		else:
			if (int(self.numberOfReports) > 1 ):
				self.multipleReports = True
		# Used for now as placeholder for ppiType
	...
	def add_ppiType(self, PPItype):
		self.ppiType.append(PPItype)
